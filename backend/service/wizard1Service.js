...
module.exports.createEstimationHeaderAtrribute = async (serviceData) => {
  try {
    let estimationHeaderAtrribute = new EstimationHeaderAtrribute({
      ...serviceData,
    });
    const findRecord = await EstimationHeaderAtrribute.find({
      estHeaderId: estimationHeaderAtrribute.estHeaderId,
    });
...
module.exports.createEstimationHeaderAtrributeCalc = async (serviceData) => {
  try {
    let estimationHeaderAtrributeCalc = new EstimationHeaderAtrributeCalc({
      ...serviceData,
    });
    const findRecord = await EstimationHeaderAtrributeCalc.find({
      estHeaderId: estimationHeaderAtrributeCalc.estHeaderId,
    });
    if (findRecord.length != 0) {
      throw new Error(
