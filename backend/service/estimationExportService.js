...
module.exports.generateExcelReport = async (reportPayload) => {
  ...
  try {
    workbook.xlsx.writeFile("./report/Estimation.xlsx").then(()=> {return true});
...
async function generateRequiredSpreadsheet(workbook, reportPayload) {
  ...
  if(getReportFlagValue("estimationDetail",reportPayload)){
    ...
    worksheet.columns = (await getEstimationRequirementData(reportPayload)).estRequirementColumns;
    worksheet.addRows((await getEstimationRequirementData(reportPayload)).estRequirementRowData);
  ...
  if(getReportFlagValue("resourcePlanning",reportPayload)){
    ...
    worksheet.addRows(await getResourcePlanningRowData(reportPayload.estimationHeaderId));
    worksheet.getRow(1).eachCell((cell) => {
      cell.font = { bold: true };
    });
  }
...
function getReportFlagValue(key,reportPayload){
  const foundElement = reportPayload.reports.find(element => element.key === key);
